...
The app now supports `ru` and `en`.

- Selected locale is stored in `localStorage` under `biologic-lims-locale`
- Translation messages live in `src/i18n/messages.ts`
- Nuxt UI component locale is synchronized through `UApp`
- Date and number formatting should use the helpers from `src/composables/useLocale.ts`
