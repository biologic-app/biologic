...
5. **Задокументировать** в корневом `CLAUDE.md` (раздел Backend): это модульный монолит = один bounded context, модули общаются только через `core` + `EventBus`, общая схема БД — намеренно.
