...
async function waitForStepElement(
  step: AppTourStep | undefined,
  attempts = 40,
) {
  if (!step?.element || typeof document === "undefined") {
    return;
...
export function getResolvedTours(
  scope: TourScope,
  context: TourContext,
): ResolvedTour[] {
  return tourRegistry
...
export async function startResolvedTour(
  tour: ResolvedTour,
  context: TourContext,
) {
...
export async function startTourById(
  scope: TourScope,
  tourId: string,
  context: TourContext,
) {
  const tour = getResolvedTours(scope, context).find(
    (item) => item.id === tourId,
  );
  if (!tour) {
    return false;
...
export async function startAutostartTour(
  scope: TourScope,
  context: TourContext,
) {
  const tour = getPrimaryTour(scope, context);
