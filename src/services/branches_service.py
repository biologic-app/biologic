...
class BranchService:
    ...
    async def create(self, payload: BranchCreateDTO) -> BranchCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return BranchCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def list(
        ...
        *,
        ...
        return BranchListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(self, entity_id: UUID, payload: BranchUpdateDTO) -> BranchUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return BranchUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(self, entity_id: UUID, reason: str | None = None) -> BranchDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return BranchDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
