...
class DirectionStatusService:
    ...
    def __init__(self, repository: DirectionStatusRepository) -> None:
        self._repository = repository
        self._crud = CRUDService(
            repository=repository,
            read_schema=DirectionStatusReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="DirectionStatus {entity_id} was not found.",
        )
    ...
    async def create(self, payload: DirectionStatusCreateDTO) -> DirectionStatusCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return DirectionStatusCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def get(
        ...
        return DirectionStatusReadEnvelopeDTO(
            data=data,
            meta=ReadMetaDTO(
                includes=includes,
    ...
    async def list(
        ...
        *,
        ...
        return DirectionStatusListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(
        self, entity_id: UUID, payload: DirectionStatusUpdateDTO
    ) -> DirectionStatusUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return DirectionStatusUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(
        self, entity_id: UUID, reason: str | None = None
    ) -> DirectionStatusDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return DirectionStatusDeleteEnvelopeDTO(
