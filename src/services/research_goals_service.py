...
class ResearchGoalService:
    ...
    def __init__(self, repository: ResearchGoalRepository) -> None:
        self._repository = repository
        self._crud = CRUDService(
            repository=repository,
            read_schema=ResearchGoalReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="ResearchGoal {entity_id} was not found.",
        )
    ...
    async def create(self, payload: ResearchGoalCreateDTO) -> ResearchGoalCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return ResearchGoalCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def get(
        ...
        return ResearchGoalReadEnvelopeDTO(
            data=data,
            meta=ReadMetaDTO(
                includes=includes,
    ...
    async def list(
        ...
        *,
        ...
        return ResearchGoalListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(
        self, entity_id: UUID, payload: ResearchGoalUpdateDTO
    ) -> ResearchGoalUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return ResearchGoalUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(
        self, entity_id: UUID, reason: str | None = None
    ) -> ResearchGoalDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return ResearchGoalDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
