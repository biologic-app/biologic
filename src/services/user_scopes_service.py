...
class UserScopeService:
    _sort_fields: ClassVar[set[str]] = {"id", "user_id", "scope_id"}

    def __init__(self, repository: UserScopeRepository) -> None:
        self._repository = repository
        self._crud = CRUDService(
            repository=repository,
            read_schema=UserScopeReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="UserScope {entity_id} was not found.",
        )
    ...
    async def create(self, payload: UserScopeCreateDTO) -> UserScopeCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return UserScopeCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def get(
        ...
        return UserScopeReadEnvelopeDTO(
            data=data,
            meta=ReadMetaDTO(
                includes=includes,
    ...
    async def list(
        ...
        *,
        ...
        return UserScopeListEnvelopeDTO(items=expanded_items, meta=meta)
    ...
    async def update(
        ...
        return UserScopeUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))
    ...
    async def delete(
        ...
        return UserScopeDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
