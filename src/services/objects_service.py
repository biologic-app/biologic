...
class ObjectService:
    ...
    def __init__(self, repository: ObjectRepository) -> None:
        ...
        self._crud = CRUDService(
            ...
            read_schema=ObjectReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="Object {entity_id} was not found.",
        )
    ...
    async def create(self, payload: ObjectCreateDTO) -> ObjectCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return ObjectCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def list(
        ...
        *,
        ...
        return ObjectListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(self, entity_id: UUID, payload: ObjectUpdateDTO) -> ObjectUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return ObjectUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(self, entity_id: UUID, reason: str | None = None) -> ObjectDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return ObjectDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
