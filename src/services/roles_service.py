...
class RoleService:
    _sort_fields: ClassVar[set[str]] = {"name", "updated_at", "key", "id", "created_at"}

    def __init__(self, repository: RoleRepository) -> None:
        self._repository = repository
        self._crud = CRUDService(
            repository=repository,
            read_schema=RoleReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="Role {entity_id} was not found.",
        )
    ...
    async def create(self, payload: RoleCreateDTO) -> RoleCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return RoleCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))

    async def get(self, entity_id: UUID, includes: list[str] | None = None) -> RoleReadEnvelopeDTO:
    ...
    async def list(
        ...
        *,
        ...
        return RoleListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(self, entity_id: UUID, payload: RoleUpdateDTO) -> RoleUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return RoleUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(self, entity_id: UUID, reason: str | None = None) -> RoleDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return RoleDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
