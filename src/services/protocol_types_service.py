...
class ProtocolTypeService:
    ...
    def __init__(self, repository: ProtocolTypeRepository) -> None:
        self._repository = repository
        self._crud = CRUDService(
            repository=repository,
            read_schema=ProtocolTypeReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="ProtocolType {entity_id} was not found.",
        )
    ...
    async def create(self, payload: ProtocolTypeCreateDTO) -> ProtocolTypeCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return ProtocolTypeCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def get(
        ...
        return ProtocolTypeReadEnvelopeDTO(
            data=data,
            meta=ReadMetaDTO(
                includes=includes,
    ...
    async def list(
        ...
        *,
        ...
        return ProtocolTypeListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(
        self, entity_id: UUID, payload: ProtocolTypeUpdateDTO
    ) -> ProtocolTypeUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return ProtocolTypeUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(
        self, entity_id: UUID, reason: str | None = None
    ) -> ProtocolTypeDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return ProtocolTypeDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
