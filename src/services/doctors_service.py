...
class DoctorService:
    ...
    def __init__(self, repository: DoctorRepository) -> None:
        ...
        self._crud = CRUDService(
            ...
            read_schema=DoctorReadDTO,
            allowed_sort_fields=self._sort_fields,
            not_found_message="Doctor {entity_id} was not found.",
        )
    ...
    async def create(self, payload: DoctorCreateDTO) -> DoctorCreateEnvelopeDTO:
        data = await self._crud.create(payload.model_dump(exclude_unset=True))
        return DoctorCreateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="create"))
    ...
    async def list(
        ...
        *,
        ...
        return DoctorListEnvelopeDTO(items=expanded_items, meta=meta)

    async def update(self, entity_id: UUID, payload: DoctorUpdateDTO) -> DoctorUpdateEnvelopeDTO:
        data = await self._crud.update(entity_id, payload.model_dump(exclude_unset=True))
        return DoctorUpdateEnvelopeDTO(data=data, meta=ActionMetaDTO(operation="update"))

    async def delete(self, entity_id: UUID, reason: str | None = None) -> DoctorDeleteEnvelopeDTO:
        await self._crud.delete(entity_id, reason=reason)
        return DoctorDeleteEnvelopeDTO(meta=DeleteMetaDTO(operation="delete", deleted=True))
