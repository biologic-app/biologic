from uuid import UUID

from fastapi import APIRouter, Body, Depends, Query, status
from starlette.requests import Request

from src.api.dependencies import get_directions_service
from src.api.v1.endpoints._helpers import parse_includes, parse_list_filters
from src.schemas import (
    DirectionCreateDTO,
    DirectionCreateEnvelopeDTO,
    DirectionDeleteEnvelopeDTO,
    DirectionListEnvelopeDTO,
    DirectionReadEnvelopeDTO,
    DirectionUpdateDTO,
    DirectionUpdateEnvelopeDTO,
)
from src.schemas.base import DeleteRequestDTO
from src.schemas.common import SortOrder
from src.services.directions_service import DirectionService

router = APIRouter(prefix="/directions", tags=["directions"])


@router.post("", response_model=DirectionCreateEnvelopeDTO, status_code=status.HTTP_201_CREATED)
async def create_directions(
    payload: DirectionCreateDTO, service: DirectionService = Depends(get_directions_service)
) -> DirectionCreateEnvelopeDTO:
    return await service.create(payload)


@router.get("/{entity_id}", response_model=DirectionReadEnvelopeDTO)
async def get_directions(
    entity_id: UUID,
    include: str | None = Query(default=None),
    service: DirectionService = Depends(get_directions_service),
) -> DirectionReadEnvelopeDTO:
    return await service.get(entity_id, includes=parse_includes(include))


@router.get("", response_model=DirectionListEnvelopeDTO)
async def list_directions(
    request: Request,
    service: DirectionService = Depends(get_directions_service),
    offset: int = Query(default=0, ge=0),
    limit: int = Query(default=15, ge=1, le=500),
    sort_by: str = Query(default="created_at"),
    sort_order: SortOrder = Query(default="desc"),
    include: str | None = Query(default=None),
) -> DirectionListEnvelopeDTO:
    exact_filters, range_filters = parse_list_filters(dict(request.query_params))
    return await service.list(
        offset=offset,
        limit=limit,
        sort_by=sort_by,
        sort_order=sort_order,
        includes=parse_includes(include),
        exact_filters=exact_filters,
        range_filters=range_filters,
    )


@router.patch("/{entity_id}", response_model=DirectionUpdateEnvelopeDTO)
async def update_directions(
    entity_id: UUID,
    payload: DirectionUpdateDTO,
    service: DirectionService = Depends(get_directions_service),
) -> DirectionUpdateEnvelopeDTO:
    return await service.update(entity_id, payload)


@router.delete("/{entity_id}", response_model=DirectionDeleteEnvelopeDTO)
async def delete_directions(
    entity_id: UUID,
    payload: DeleteRequestDTO | None = Body(default=None),
    service: DirectionService = Depends(get_directions_service),
) -> DirectionDeleteEnvelopeDTO:
    reason = payload.reason if payload is not None else None
    return await service.delete(entity_id, reason=reason)
