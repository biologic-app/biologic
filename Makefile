...
.PHONY: help install lint format test typecheck build e2e sdk-generate \
        be-dev be-test be-lint be-format be-audit be-seed-data be-import-legacy be-analyze-orm-queries \
...
help:
	...
	@echo "  backend: be-dev be-test be-lint be-format be-audit be-seed-data be-import-legacy be-k6-scenarios"
...
# Legacy MySQL 5.1 → canonical PostgreSQL ETL. Dry-run is the default;
# use LEGACY_ETL_ARGS=--apply only against an explicitly chosen target DB.
be-import-legacy:
	cd backend && uv run python -m scripts.import_legacy_mysql $(LEGACY_ETL_ARGS)
